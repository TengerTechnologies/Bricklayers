    ...
    for line in lines:
        ...
        if line.startswith("G1 Z"):
            ...
            if z_match:
                ...
                logging.debug(f"Layer {current_layer} detected at Z={current_z:.3f}")
        ...
        if perimeter_type == "internal" and line.startswith("G1") and "X" in line and "Y" in line and "E" in line:
            ...
            if not inside_perimeter_block:
                ...
                logging.debug(f"Perimeter block #{perimeter_block_count} detected at layer {current_layer}")
                ...
                if perimeter_block_count % 2 == 1:  # Apply Z-shift to odd-numbered blocks
                    ...
                    logging.debug(f"Inserting G1 Z{adjusted_z:.3f} for shifted perimeter block #{perimeter_block_count}")
                    ...
                    logging.debug(f"Inserting G1 Z{current_z:.3f} for non-shifted perimeter block #{perimeter_block_count}")
            ...
            if is_shifted:
                ...
                if e_match:
                    ...
                    if current_layer == 0:  # First layer
                        ...
                        logging.debug(f"Multiplying E value by 1.5 on first layer (shifted block): {e_value:.5f} -> {new_e_value:.5f}")
                        ...
                        logging.debug(f"Multiplying E value by 0.5 on last layer (shifted block): {e_value:.5f} -> {new_e_value:.5f}")
                        line = re.sub(r'E[-\d.]+', f'E{new_e_value:.5f}', line).strip()
                        line += f" ; Adjusted E for last layer, block #{perimeter_block_count}\n"
                    else: 
                        new_e_value = e_value * extrusion_multiplier
                        logging.debug(f"Multiplying E value by extrusionMultiplier: {extrusion_multiplier:.5f}")
                        line = re.sub(r'E[-\d.]+', f'E{new_e_value:.5f}', line).strip()
